...
local function NotificacaoNightMystic(titulo, mensagem)
    ...
    if not success then
        warn("[hiruz] Error displaying notification)")
    end
end
...
-- ALTURA ĂNICA AJUSTĂVEL DO MOB
...
--==================================================
-- VARIĂVEIS DE CONTROLE NECESSĂRIAS
--==================================================
_G = _G or {}
...
--==================================================
-- FUNĂ‡ĂƒO: VERIFICA SE QUALQUER FARM ESTĂ ATIVO
--==================================================
...
--==================================================
-- FUNĂ‡ĂƒO: IGNORA MOBS INDESEJADOS
--==================================================
...
-- =======================
-- TWEEN AJUSTADO POR DISTĂ‚NCIA
-- =======================

-- [[ VARIĂVEIS PARA O SEU INPUT ]] --
getgenv().TweenSpeedFar = 300   -- Velocidade PadrĂ£o (Longe)
...
-- Garantir que nĂ£o estĂ¡ ancorado  
...
-- ===============================  
--  SE ESTIVER ATĂ‰ 15 STUDS â†’ USA A VELOCIDADE DE PERTO
--  CASO CONTRĂRIO â†’ USA A VELOCIDADE PADRĂƒO
-- ===============================  
...
QuestCheck = function()
    ...
    -- [CORREĂ‡ĂƒO] Fixar level dentro do limite da missĂ£o
    if World1 and I > 699 then
        I = 650 -- ForĂ§a a missĂ£o do Galley Captain (Lv 650)
    end
    
    if World2 and I > 1499 then
        I = 1450 -- ForĂ§a a missĂ£o do Water Fighter (Lv 1450)
    end
...
elseif I == 1650 or I <= 1699 then
    ...
    PosM = CFrame.new(4674.92676, 1134.82654, 996.308838, 0.731321394, -0, -0.682033002, 0, 1, -0, 0.682033002, 0, 0.731321394)
		...
		elseif I >= 2600 and I <= 2624 then
			...
			-- CĂ³digo para executar SOMENTE se jĂ¡ estiver na ilha
			Mon = "Reef Bandit"
			Qdata = 1
...
MaterialMon = function()
    ...
    -- Verifica se vocĂª estĂ¡ longe da entrada. Se estiver, pede para entrar.
...
imageButton.Image = "rbxassetid://123613996022560"
...
-- VariĂ¡veis para arrastar
local dragging = false
local dragInput
local dragStart
local startPos

-- FunĂ§Ă£o para atualizar posiĂ§Ă£o
...
-- Atualizar posiĂ§Ă£o durante arrasto
...
Discord:AddDiscordInvite({
    ...
    Logo = "rbxassetid://123613996022560",
    Invite = "https://discord.com/invite/chuoihub"
})
local credits = Discord:AddParagraph({
    Title = "Credits For Someone Peoples",
    Desc = ""
})
credits:SetDesc("Cuong Meo")
...
-- VariĂ¡vel para guardar o mob de Level Farm
local CurrentMobLevel = nil
local CurrentMobLevelSystem = nil

-- Define o limite de distĂ¢ncia para teleporte condicional (em studs)
local TELEPORT_DISTANCE_THRESHOLD = 15 

-- FunĂ§Ă£o auxiliar para teleporte condicional
...
----------------------------------------------------------------------------
-- 1. UI: DROPDOWN + TOGGLES (Coloque isso na seĂ§Ă£o da sua UI)
----------------------------------------------------------------------------
...
Farm:AddToggle({
    ...
    Callback = function(v)
        ...
        -- Reset total das variĂ¡veis de controle para evitar conflitos
        _G.Level = false  
        _G.AutoFarm_Bone = false  
        _G.AutoFarm_Cake = false  
        _G.AutoTyrant = false 
        ...
        if v then  
            ...
            elseif _G.SelectedFarmMode == "Tyrant Of The Skies" then  
                _G.AutoTyrant = true -- Ativa a lĂ³gica do Tyrant
            end  
        end
...
----------------------------------------------------------------------------
-- 2. LĂ“GICA DO TYRANT (FARM, QUESTS E BOSS)
----------------------------------------------------------------------------

-- ConfiguraĂ§Ăµes
local FarmHeight = 45 -- Altura segura para nĂ£o bugar no mob

-- NoClip ReforĂ§ado (Para nĂ£o travar nas paredes/mobs)
spawn(function()
...
-- FunĂ§Ă£o MĂ¡gica: Seleciona o Mob/Quest exato pelo NĂ­vel
...
spawn(function()
    local VaseIndex = 1 -- Ăndice para controlar qual vaso atacar
    
    while task.wait() do
        -- SĂ³ roda se a toggle estiver ativada E o modo for Tyrant
        if _G.AutoTyrant and _G.StartFarm then
            pcall(function()
                local plr = game.Players.LocalPlayer
                ...
                elseif eyes == 4 then
                    ...
                    -- CORREĂ‡ĂƒO: Usa o VaseIndex para ciclar entre os vasos ao invĂ©s de resetar o loop
    ...
    while task.wait() do
        if _G.AutoFarm_Bone and _G.StartFarm then
            pcall(function()
                ...
                local npcPos = CFrame.new(-9516.99, 172.01, 6078.46) -- PosiĂ§Ă£o do NPC de Bones
                ...
                if _G.AcceptQuest and not QuestUI.Visible then      
                    ...
                    if distToNPC > 5 then
                        ...
                        -- Tenta pegar a quest baseada no seu level (opcional: deixei aleatĂ³rio como no seu original)
                ...
                -- [PASSO 2] SE JĂ TEM QUEST OU NĂƒO QUER ACEITAR, VAI FARMAL
                else
                    ...
                    if mob then      
                        ...
                        -- Se nĂ£o achar mobs, vai para o spot de spawn deles
...
-- ConfiguraĂ§Ă£o da DistĂ¢ncia MĂ¡xima (em studs)
...
Farm:AddToggle({
    ...
    Callback = function(I)
        ...
        -- 2. Guarda na tabela de configuraĂ§Ăµes para o Save
...
spawn(function()
    while task.wait() do
        pcall(function()
            ...
            -- đŸ” 1. Procurar inimigos no WORKSPACE
            ...
            -- đŸ” 2. Procurar inimigos no REPLICATEDSTORAGE
            if ReplicatedStorage:FindFirstChild("Enemies") then
                for _, e in pairs(ReplicatedStorage.Enemies:GetChildren()) do
                    if e:IsA("Model") and e:FindFirstChild("Humanoid") then
                        -- Se nĂ£o achou nenhum no workspace ainda
...
Farm:AddToggle({
    ...
    Callback = function(Value)
        ...
        -- 2. Guarda a informaĂ§Ă£o na tabela de salvamento
    ...
    while task.wait(Sec) do
        pcall(function()
            ...
            if Core and Core:FindFirstChild("Humanoid") and Core.Humanoid.Health > 0 then
                ...
                -- Core nĂ£o encontrado â†’ vai pro meio da factory
                _tp(FactoryPos)
            end
        end)
    end
...
-- BotĂ£o Auto Collect Chest
Farm:AddToggle({
    Name = "Auto Collect Chest",
    Description = "",
    -- 1. Carrega o estado salvo (ou false por padrĂ£o)
...
Farm:AddToggle({
    ...
    Description = "",
    -- 1. Carrega se estava ligado ou nĂ£o
...
Setting:AddButton({
    Name = "Salvar ConfiguraĂ§Ăµes Agora",
    Description = "Cria um backup manual das tuas opĂ§Ăµes atuais",
    Callback = function()
        -- Verifica se a funĂ§Ă£o existe antes de chamar
        if SaveSettings then
            SaveSettings()
            
            -- NotificaĂ§Ă£o Universal (Funciona sem a lib Fluent)
            game.StarterGui:SetCore("SendNotification", {
                Title = "Banana Hub",
                Text = "ConfiguraĂ§Ăµes salvas com sucesso!",
                Duration = 5
            })
        else
            warn("Erro: FunĂ§Ă£o SaveSettings nĂ£o encontrada!")
        end
    end
})

Setting:AddButton({
    Name = "Resetar ConfiguraĂ§Ăµes",
    Description = "Apaga o ficheiro de save e volta ao padrĂ£o",
    Callback = function()
        -- Usa a variĂ¡vel FullPath que foi definida lĂ¡ no topo do script
        if isfile and isfile(FullPath) then
            delfile(FullPath)
            _G.SaveData = {} -- Limpa a tabela na memĂ³ria ram
            
            -- NotificaĂ§Ă£o Universal
            game.StarterGui:SetCore("SendNotification", {
                Title = "Banana Hub",
                Text = "ConfiguraĂ§Ăµes resetadas! Re-execute o script.",
                Duration = 5
            })
        else
            game.StarterGui:SetCore("SendNotification", {
...
Setting:AddButton({
    ...
    Callback = function()
        ...
        -- đŸ”´ PARA O TWEEN (usa a variĂ¡vel que JĂ EXISTE)
        shouldTween = false
...
Setting:AddToggle({
	Name = "Auto Attack",
	Description = "",
	-- 1. Carrega o estado salvo ou inicia como true (padrĂ£o original)
...
-- DefiniĂ§Ăµes de ServiĂ§os e MĂ³dulos NecessĂ¡rios
...
-- =========================================================
-- CONFIGURAĂ‡ĂƒO DA NOTIFICAĂ‡ĂƒO CUSTOMIZADA (FULL BLACK)
-- =========================================================
local LogoID = "rbxassetid://123613996022560"
...
-- ConfiguraĂ§Ăµes Iniciais Globais (serĂ£o sobrescritas pelo Save se existirem)
...
Others:AddDropdown({
    ...
    -- Carrega a Ăºltima vara salva, ou usa "Fishing Rod"
...
Others:AddDropdown({
    ...
    -- Carrega a Ăºltima isca salva, ou usa "Basic Bait"
...
Others:AddToggle({
    Name = "Auto Fishing",
    Description = "LanĂ§a e puxa a vara automaticamente",
    -- Carrega se estava ligado
    ...
    while task.wait(0.5) do
        if _G.AutoFishing then
            pcall(function()
                ...
                -- Verifica se a vara selecionada estĂ¡ equipada
                if _G.SelectedRod and (not equippedTool or equippedTool.Name ~= _G.SelectedRod) then
                    ...
                    if rodInBag then
                        ...
                        return -- Para a execuĂ§Ă£o aqui para nĂ£o dar erro tentando pescar sem vara
                    end
                end
                ...
                if equippedTool then
                    ...
                    elseif serverState == "Biting" then
                        ...
                        -- [[ NOTIFICAĂ‡ĂƒO PRETA AO PEGAR PEIXE ]]
...
Others:AddToggle({
    ...
    Description = "Pega missĂ£o com o NPC Angler",
    -- Carrega se estava ligado
...
spawn(function()
 ...
 Others:AddToggle({
    ...
    Description = "",
    -- 1. Carrega o estado salvo ou inicia como true (padrĂ£o do seu script)
...
-- AUTO YAMA (EXATAMENTE COMO VOCĂ ENVIou)
 Others:AddToggle({
...
local z5 = {
    ...
    -- ConfiguraĂ§Ă£o de NĂ­vel de Perigo (SĂ“ SEA 3)
    if World3 then
        local H5 = {
...
spawn(function()
    while wait() do
        if _G.SailBoats then
            pcall(function()
                local I = CheckBoat();
                -- CORREĂ‡ĂƒO: Compra barco mesmo se houver inimigos, para nĂ£o travar no spawn
                if not I then
...
-- // VARIĂVEL DE VELOCIDADE PADRĂƒO // --
_G.SetSpeedBoat = 300
...
task.spawn(function()
    game:GetService("RunService").RenderStepped:Connect(function()
        if _G.SpeedBoat then
            pcall(function()
                local plr = game.Players.LocalPlayer
                -- Verifica se o jogador estĂ¡ sentado
                if plr.Character and plr.Character:FindFirstChild("Humanoid") and plr.Character.Humanoid.Sit then
                    for _, boat in pairs(workspace.Boats:GetChildren()) do
                        ...
                        if seat then
                            -- Aplica a velocidade e melhora a direĂ§Ă£o
...
-- FunĂ§Ă£o OTIMIZADA para usar todas as skills de uma categoria antes de sair
    ...
    if _G.SelectedSkills[category] then
        ...
        for _, skill in ipairs(keys) do
            if _G.SelectedSkills[category][skill] then
                ...
                task.wait(0.35) -- Espera um pouco entre cada skill para nĂ£o bugar
            end
        end
    end
end
                    ...
                    if e:FindFirstChild("HumanoidRootPart") and (e:FindFirstChild("Health") and e.Health.Value > 0) then
                        repeat
                            ...
                            if plr:DistanceFromCharacter(e.HumanoidRootPart.CFrame.Position) <= 500 then
                                ...
                                if CheckF() then
                                    -- Se estiver transformado, foca sĂ³ em Blox Fruit
                                    ExecuteSkills("Blox Fruit")
                                else
                                    -- SequĂªncia completa: Melee -> Sword -> Fruit -> Gun
                                    ExecuteSkills("Melee")
                                    ExecuteSkills("Sword")
                                    ExecuteSkills("Blox Fruit")
                                    ExecuteSkills("Gun")
...
Event:AddToggle({
    ...
    Callback = function(I)
        ...
        -- 2. Guarda na memĂ³ria de salvamento
...
Event:AddToggle({
    ...
    Description = "",
    -- 1. Carrega se o trade automĂ¡tico estava ligado
...
Event:AddToggle({
    ...
    Description = "Voa atĂ© o ponto mais alto da Mirage (Ideal para olhar a Lua)",
    -- 1. Carrega o estado salvo
            ...
            pcall(function()
                ...
                if mystic then
                    for _, e in pairs(mystic:GetChildren()) do
                        -- A engrenagem na Mirage Ă© um MeshPart chamado "Part"
...
Event:AddToggle({
    ...
    Description = "Deixa os obstĂ¡culos da Mirage transparentes",
    ...
    Callback = function(I)
        ...
        -- Se desligar, tentamos resetar a transparĂªncia (Opcional)
        if not I then
            pcall(function()
                local mystic = workspace.Map:FindFirstChild("MysticIsland")
                if mystic then
...
-- Loop de TransparĂªncia
task.spawn(function()
    while task.wait(1) do -- Intervalo de 1 segundo para nĂ£o pesar
        if _G.can then
            pcall(function()
                local mystic = workspace.Map:FindFirstChild("MysticIsland")
                if mystic then
                    for _, e in pairs(mystic:GetChildren()) do
                        if e.Name == "Part" then
                            if e:IsA("MeshPart") then
                                e.Transparency = 0 -- MantĂ©m o que Ă© importante visĂ­vel
                            else
                                e.Transparency = 1 -- Esconde o resto (paredes/Ă¡rvores)
                            end
                        end
                    end
                end
...
Event:AddToggle({
    ...
    Description = "Voa atĂ© o vendedor de frutas avanĂ§ado na Mirage",
    -- 1. Carrega se o teleporte estava ativo
...
task.spawn(function()
    while task.wait(1) do -- 1 segundo Ă© suficiente para verificar o NPC
        if _G.Addealer then
            pcall(function()
...
-- ConfiguraĂ§Ăµes de UI e Toggle
Event:AddToggle({
...
spawn(function()
    while wait(Sec) do
        pcall(function()
            if _G.CraftVM then
                -- Verifica se jĂ¡ possui o item final
                if GetM("Volcanic Magnet") < 1 then
                    
                    -- CASO 1: JĂ¡ tem os materiais, tenta craftar/notificar
                    ...
                    elseif GetM("Scrap Metal") < 10 then
                        ...
                        if I then
                            ...
                            -- Teleporte padrĂ£o caso nĂ£o ache o inimigo
                            _tp(CFrame.new(-13206.452148438, 425.89199829102, -7964.5537109375));
                        end;
...
Event:AddToggle({
    ...
    Callback = function(I)
        ...
        -- 2. Salva a configuraĂ§Ă£o no arquivo
        _G.SaveData["PrehistoricFinder_Save"] = I
        SaveSettings()
    end,
})

-- O Loop (spawn) deve ficar fora do AddToggle para nĂ£o ser criado vĂ¡rias vezes
task.spawn(function()
    while task.wait() do
        if _G.Prehis_Find then
            pcall(function()
                -- Se a ilha NĂƒO foi encontrada ainda
                if not workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island", true) then
                    
                    local I = CheckBoat()

                    -- [CORREĂ‡ĂƒO] Verifica se o barco estĂ¡ muito longe
                    ...
                    if workspace.Map:FindFirstChild("PrehistoricIsland", true) or workspace._WorldOrigin.Locations:FindFirstChild("Prehistoric Island", true) then
                        ...
                        if promptPart and promptPart:FindFirstChild("ProximityPrompt") then
                            if plr:DistanceFromCharacter(promptPart.Position) <= 150 then
                                ...
                                -- SimulaĂ§Ă£o de tecla E para garantir
...
Event:AddToggle({
    ...
    Callback = function(I)
        -- Define as variĂ¡veis globais
        _G.PrehistoricEvent = I
        _G.Prehis_Skills = I
        
        -- 2. Salva a configuraĂ§Ă£o
        _G.SaveData["AutoEventPrehistoric_Save"] = I
        SaveSettings()

        -- 3. OtimizaĂ§Ă£o do NoClip (SĂ³ roda se o Toggle estiver ON)
        if I then
            local RunService = game:GetService("RunService")
            task.spawn(function()
                -- Usamos um loop controlado para nĂ£o sobrecarregar o processador
...
--------------------------------------------------------------------
spawn(function()
    while task.wait(1.5) do -- Aumentei para 1.5s (NĂ£o precisa ser instantĂ¢neo)
        if _G.Prehis_Skills then
            pcall(function()
                -- Foca APENAS na ilha, nĂ£o no jogo todo (Isso remove o lag)
                ...
                if MapIsland then
                    ...
                    -- Loop ĂNICO para limpar tudo dentro da ilha
...
--------------------------------------------------------------------
spawn(function()
    while task.wait() do -- O wait() vazio aqui Ă© necessĂ¡rio para a velocidade do ataque
        if _G.Prehis_Skills then
            pcall(function()
                local plr = game.Players.LocalPlayer
                ...
                if workspace.Enemies:FindFirstChild("Lava Golem") then
                    ...
                    pcall(function() 
                        -- Assumindo que vocĂª tem essa funĂ§Ă£o global ou no script
                         if GetConnectionEnemies then
                            enemy = GetConnectionEnemies("Lava Golem") 
                         else
                            -- Fallback caso nĂ£o tenha a funĂ§Ă£o
                    ...
                    if enemy and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
                        repeat
                            task.wait() -- Importante para nĂ£o congelar
                            ...
                            if layer:FindFirstChild("At0") and layer.At0:FindFirstChild("Glow") and layer.At0.Glow.Enabled == true then
                                ...
                                repeat
                                    ...
                                    if plr:DistanceFromCharacter(layer.CFrame.Position) <= 200 then
                                        ...
                                        -- Adicionei um pequeno delay entre as teclas para nĂ£o sobrecarregar o InputManager
            ...
            task.wait(1) -- Se a funĂ§Ă£o estiver desligada, espera 1s para nĂ£o gastar processamento Ă  toa
        end
    end
end)
--------------------------------------------------------------------
-- BOTĂƒO MANUAL DE APAGAR LAVA
--------------------------------------------------------------------
...
task.spawn(function()
    while task.wait() do
        pcall(function()
            ...
            -- Prioridade 2: Dino Bones (Se nĂ£o estiver pegando ovo)
...
-- FUNĂ‡ĂƒO: pegar inimigo mais prĂ³ximo dentro de uma lista
...
-- FUNĂ‡ĂƒO: verificar se ainda existe mob vivo
...
-- CĂ“DIGO MODIFICADO COM SAVE
Maestry:AddToggle({
    Name = "Auto Farm Mastery Fruit",
    Description = "",
    -- 1. Usa GetSetting para pegar o valor salvo (ou false se nĂ£o tiver salvo)
...
Dojo:AddToggle({
	Name = "Fly",
	Description = "Fly controlado pelo analĂ³gico ( Subida AutomĂ¡tica )",
	Default = false,
	Callback = function(v)
		_G.Fly = v
	end,
...
local yForce = 50       -- forĂ§a de subida (AJUSTE AQUI)
...
local currentFont = Enum.Font.Arial -- Fonte padrĂ£o inicial
...
-- // AUTO STATS (Adicionado Ă  aba VI_S conforme solicitado) // --
...
-- Usamos um valor grande (como 9999999) para simular o gasto de 'todos' os pontos disponĂ­veis.
-- Assumimos que a funĂ§Ă£o 'statsSetings' irĂ¡ apenas gastar o mĂ¡ximo de pontos que o jogador realmente tem.
...
spawn(function()
	while wait(Sec) do
		pcall(function()
			-- Note: No seu cĂ³digo original era 'Auto_DevilFruit', mas no menu era 'Fruit'. Corrigi para usar a mesma variĂ¡vel do menu.
...
--==================================================
-- TOGGLE ĂNICO (PRIMEIRO NO CĂ“DIGO)
--==================================================
    ...
    Callback = function(Value)
        ...
        if Value then
            ...
            if targetIsland then
                spawn(function()
                    -- 1. Subida InstantĂ¢nea (700 studs acima de onde vocĂª estĂ¡ agora)
                    Root.CFrame = Root.CFrame * CFrame.new(0, 700, 0)
                    wait(0.1) -- Pequena pausa para o motor fĂ­sico processar
                    
                    -- 2. Viagem (Mantendo a altura de 700 studs acima do destino)
                    local destination = targetIsland.CFrame * CFrame.new(0, 700, 0)
                    
                    -- Repete o movimento atĂ© chegar perto do destino (eixo X e Z)
                    repeat
                        wait()
                        _tp(destination)
                    until not _G.Teleport or (Root.Position - destination.p).Magnitude < 10
                    
                    -- 3. Descida InstantĂ¢nea (Teleporta para a superfĂ­cie da ilha)
                    if _G.Teleport then
                        Root.CFrame = targetIsland.CFrame * CFrame.new(0, 5, 0) -- 5 studs para nĂ£o bugar no chĂ£o
                    end
                    
                    -- 4. Desligar AutomĂ¡tico
                    _G.Teleport = false
                    -- Nota: Para o botĂ£o visual do menu "desligar" sozinho, 
                    -- depende da biblioteca que vocĂª estĂ¡ usando (ex: Fluent, Orion, Rayfield).
	...
	while wait(0.1) do
		if _G.FarmMastery_S then
			pcall(function()
				...
				-- SĂ³ usa o CFrame SE NĂƒO EXISTIR MOB
				if not Target then
        ...
        if I then
            spawn(function()
                while _G.Tp_LgS do
                    ...
                    wait(1) -- Delay para nĂ£o travar o jogo
                end
            end)
        end
    end,
            ...
            if _G.AutoKeyRen then
                ...
                if plr.Backpack:FindFirstChild(K[3]) or plr.Character:FindFirstChild(K[3]) then
                    ...
                    if enemy then
                        ...
                        -- Teleporta para o spawn do inimigo caso nĂ£o encontre
		...
		task.spawn(function()
			while _G.AutoBuyChip do
				...
				if not GetBP("Special Microchip") then
					...
					-- Usar frutas vĂ¡lidas para comprar chip
...
Fruit:AddToggle({
    ...
    Description = "Inicia a Raid automaticamente e aguarda 10s entre verificaĂ§Ăµes",
    Default = false,
    Callback = function(I)
        _G.Auto_StartRaid = I;
    end,
...
spawn(function()
    -- O loop agora espera 10 segundos para cada execuĂ§Ă£o
    while true do 
        task.wait(10)
        
        if _G.Auto_StartRaid then
            pcall(function()
                -- SĂ³ tenta iniciar se a Raid nĂ£o estiver ativa (Timer invisĂ­vel)
                if plr.PlayerGui.Main.TopHUDList.RaidTimer.Visible == false then
                    ...
                    if GetBP("Special Microchip") then
                        ...
                        elseif World3 then
                            -- 1. Tenta usar o portal via InvokeServer (apenas uma tentativa rĂ¡pida)
                            ...
                            -- 2. Fallback: Se o portal falhou ou o player nĂ£o tem acesso, 
                            -- o script usa o seu _tp para ir atĂ© a posiĂ§Ă£o do botĂ£o.
                            _tp(CFrame.new(-5033.50879, 315.014252, -2947.77539));
                            
                            -- 3. Aciona o botĂ£o da Raid
...
Fruit:AddToggle({
    ...
    -- 1. Carrega se o giro automĂ¡tico estava ligado
...
Fruit:AddToggle({
    ...
    -- 1. Carrega se o armazenamento automĂ¡tico estava ligado
...
Setting:AddButton({
    ...
    Callback = function()
        ...
        -- SE O SCRIPT AINDA NĂƒO TĂ RODANDO
...
NotificacaoNightMystic("Binn redz", "âœ… All systems loaded!")
